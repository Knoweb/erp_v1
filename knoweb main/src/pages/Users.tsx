...
const Users = () => {
  ...
  const getRoleBadgeStyle = (role: string) => {
    switch (role) {
      ...
      case 'ROLE_INV_MOLDING':
      case 'ROLE_INV_ASSEMBLE':
      case 'ROLE_INV_PRIMARY':
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200">
                    ...
                    <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-widest">Role & Access</th>
                  ...
                  {filteredUsers.map(user => (
                    <tr key={user.id} className="hover:bg-slate-50/80 transition-colors group">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-full bg-gradient-to-br from-indigo-100 to-indigo-200 flex items-center justify-center text-indigo-700 font-bold border border-indigo-200 shadow-sm flex-shrink-0">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-wrap gap-1.5">
                          {user.roles && user.roles.length > 0 ? user.roles.map((r, i) => (
                            <span key={i} className={`flex items-center gap-1 text-[11px] font-bold px-2.5 py-1 rounded-full border ${getRoleBadgeStyle(r)}`}>
                              <Shield className="w-3 h-3" />
                              {r.replace('ROLE_', '')}
                            </span>
                          )) : (
                            <span className="flex items-center gap-1 text-[11px] font-bold px-2.5 py-1 rounded-full border bg-slate-100 text-slate-600 border-slate-200">
                              <Shield className="w-3 h-3" />
                              USER
                            </span>
                          )}
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-1.5 text-xs font-medium text-slate-600 bg-slate-100 px-3 py-1.5 rounded-lg w-max border border-slate-200">
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => confirmDelete(user)} className="p-2 text-rose-500 bg-rose-50 hover:bg-rose-100 hover:text-rose-600 rounded-lg transition-colors border border-rose-100 shadow-sm" title="Revoke Access">
