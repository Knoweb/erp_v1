  ...
  const validateForm = () => {
    ...
    const activeRows = rows.filter((row, idx) => {
      const isEmpty = isServiceMode
        ...
        : !row.poItemSelectionKey && !row.description && !row.account && !row.quantity && !row.unitPrice;
    ...
    if (activeRows.length === 0) {
      ...
      rows.forEach((row, index) => {
        ...
        const errorsInRow = {};
        if (!isServiceMode && !row.poItemSelectionKey) {
        ...
        } else if (!resolveAccountCode(row)) {
    ...
    if (roundedTotal - roundedPaid > 0.01 && !dueDate) {
      newErrors.dueDate = "Promised date is required for balance due";
    }
        ...
        <div>
          ...
          <select
            ...
            className={`w-full px-3 py-2 border rounded-lg focus:ring-2 text-sm sm:text-base ${
              errors.supplier ? "border-red-500 focus:ring-red-500 bg-red-50" : "border-gray-300 focus:ring-blue-500"
            }`}
