      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <div className="space-y-4">
            <div>
              <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wider">Supplier Information</h3>
              <p className="mt-2 text-lg font-medium text-gray-900">{purchase.supplierName || (purchase.supplier && purchase.supplier.name) || 'N/A'}</p>
            </div>
            <div>
        ...
        <div className="border-t border-gray-200 px-6 py-6">
          <h3 className="text-lg font-bold text-gray-800 mb-4 border-b-2 border-blue-400 inline-block">Item Details</h3>
        ...
        <div className="border-t border-gray-200 px-6 py-6 bg-gray-50 flex flex-col md:flex-row justify-end">
          <div className="w-full md:w-1/3 space-y-3">
            <div className="flex justify-between text-sm">
              ...
              <span className="text-gray-900 font-semibold">{Number(purchase.subtotal).toFixed(2)}</span>
            </div>
            
            <div className="flex justify-between text-sm">
              <span className="text-gray-600 font-medium">Freight</span>
              <span className="text-gray-900 font-semibold">{Number(purchase.freight).toFixed(2)}</span>
            </div>
            ...
            <div className="flex justify-between text-sm border-t border-gray-200 pt-2">
              ...
              <span className="text-gray-900 font-semibold">{Number(purchase.taxAmount || 0).toFixed(2)}</span>
            </div>
            ...
            <div className="flex justify-between text-lg border-t border-gray-300 pt-3 mt-1">
              ...
              <span className="text-blue-700 font-bold">{Number(purchase.total).toFixed(2)}</span>
            </div>
            
            <div className="flex justify-between text-sm border-t border-gray-200 pt-2 mt-2">
              <span className="text-green-600 font-medium">Amount Paid</span>
              <span className="text-green-700 font-semibold">{Number(purchase.amountPaid || 0).toFixed(2)}</span>
            </div>
            <div className="flex justify-between text-sm border-t border-gray-200 pt-2">
