...
import ginumLogo from "../../assets/ginuma-logo.jpeg";
  ...
  return (
    <div
      className={`bg-gray-50 min-h-screen px-1.5 w-72 fixed left-0 top-0 
            transition-transform duration-700 ${isVisible ? "translate-x-0" : "-translate-x-full"
        } max-h-screen overflow-y-auto`}
    >
      <div className="sticky top-0 shadow px-10 bg-gray-50 z-10 py-3">
        <button
          onClick={() => navigate("/app/dashboard")}
          className="w-full flex items-center justify-center cursor-pointer focus:outline-none"
          title="Go to Dashboard"
        >
          <img src={ginumLogo} alt="Ginum" className="h-12 w-auto object-contain" />
        </button>
      </div>
                ...
                <li>
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-6 py-2 
                                      rounded-2xl text-gray-700 hover:bg-sky-200 cursor-pointer ${location.pathname.startsWith(item.path)
                  ...
                  >
                    ...
                    {hasSubItems && (
                      <div
                        className={`transform transition-transform duration-500 ease-in-out ${expandedTabs.has(item.id) ? "rotate-180" : "rotate-0"
                          }`}
                      >
                          ...
                          <li key={subItem.id}>
                            <button
                              ...
                              className={`w-full text-left px-4 py-2 rounded-lg 
                                                        text-gray-600 hover:bg-sky-100 ${location.pathname ===
                                  subItem.path
