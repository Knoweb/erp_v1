...
const SuperAdminSideBar = ({ isCollapsed }) => {
  ...
  return (
    <div className="flex">
      {/* Sidebar */}
      <div
        className={`h-screen bg-white transition-all duration-300 flex flex-col fixed ${isCollapsed ? "w-20" : "w-64"
          } px-4 py-5`}
      >
        {/* Logo */}
        <div className="flex items-center justify-center mb-6">
          ...
          >
            <img
              src={ginumLogo}
              alt="Ginum Logo"
              className={`${isCollapsed ? "w-7 h-7" : "h-10 w-auto"} object-contain`}
            />
          </button>
        </div>
        ...
        <div className="mb-6">
          ...
          {openCompany && !isCollapsed && (
            <div className="pl-6">
              <button
                className={`w-full text-left p-2 my-1 ${activePage === "companies" ? "text-blue-500" : "text-gray-600"
                  } hover:text-blue-500`}
                onClick={() => {
          ...
          {openRequests && !isCollapsed && (
            <div className="pl-6">
              <button
                className={`w-full text-left p-2 my-1 ${activePage === "requests" ? "text-blue-500" : "text-gray-600"
                  } hover:text-blue-500`}
                onClick={() => {
